              ...
              >
                ...
                <div class="card-top">
                  <span class="type-pill" :class="'type-' + r.typeSupport?.toLowerCase()">
                    {{ icone(r.typeSupport) }} {{ r.typeSupport }}
                  </span>
      ...
      <div class="modal">
        ...
        <div class="modal-top">
          <span class="type-pill" :class="'type-' + ressourceDetail.typeSupport?.toLowerCase()">
            {{ icone(ressourceDetail.typeSupport) }} {{ ressourceDetail.typeSupport }}
          </span>
        ...
        <div class="modal-grid">
          ...
          <div v-if="ressourceDetail.auteurPartenaire" class="modal-item">
            ...
            <span class="modal-val">✍️ {{ ressourceDetail.auteurPartenaire }}</span>
          </div>
        </div>
...
.badge-type { padding: 5px 14px; border-radius: 999px; font-size: 0.8rem; font-weight: 700; }
...
.badge-count { padding: 5px 14px; border-radius: 999px; font-size: 0.8rem; font-weight: 600; background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.8); }
...
.btn-catalogue { display: inline-block; background: #D4FF00; color: #0b0f2a; font-weight: 700; padding: 12px 24px; border-radius: 10px; text-decoration: none; }
...
.ressource-card:hover { background: rgba(255,255,255,0.09); border-color: rgba(212,255,0,0.2); transform: translateY(-4px); box-shadow: 0 12px 32px rgba(0,0,0,0.4); }
.ressource-card:hover::before { opacity: 1; }

.card-top { display: flex; justify-content: space-between; align-items: center; gap: 8px; }

.type-pill { padding: 3px 10px; border-radius: 999px; font-size: 0.68rem; font-weight: 800; color: white; }
...
.card-meta { display: flex; flex-wrap: wrap; gap: 8px; font-size: 0.75rem; color: rgba(255,255,255,0.4); }
.card-tags { display: flex; flex-wrap: wrap; gap: 5px; }
.tag { background: rgba(212,255,0,0.08); color: rgba(212,255,0,0.7); border: 1px solid rgba(212,255,0,0.15); padding: 2px 8px; border-radius: 6px; font-size: 0.67rem; font-weight: 600; }

.card-footer { display: flex; justify-content: space-between; align-items: center; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.07); margin-top: auto; }
...
/* STATES */
.state-center { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 60vh; text-align: center; padding: 40px; gap: 12px; }
.spinner { width: 44px; height: 44px; border: 3px solid rgba(255,255,255,0.1); border-top-color: #D4FF00; border-radius: 50%; animation: spin 0.8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.error-icon { font-size: 3rem; }
.state-center h3 { font-size: 1.3rem; margin: 0; }
.state-center p { color: rgba(255,255,255,0.5); margin: 0; }
.btn-retour { background: #D4FF00; color: #0b0f2a; font-weight: 700; padding: 12px 24px; border: none; border-radius: 10px; cursor: pointer; font-size: 0.9rem; }

/* MODALE */
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.65); display: flex; align-items: center; justify-content: center; z-index: 300; padding: 20px; }
.modal { background: #0f1535; border: 1px solid rgba(255,255,255,0.12); border-radius: 20px; width: 100%; max-width: 620px; max-height: 90vh; overflow-y: auto; padding: 28px; position: relative; box-shadow: 0 24px 64px rgba(0,0,0,0.6); }
.modal-close { position: absolute; top: 16px; right: 16px; background: rgba(255,255,255,0.08); border: none; color: rgba(255,255,255,0.6); font-size: 16px; cursor: pointer; border-radius: 50%; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; }
...
.modal-label { display: block; font-size: 0.7rem; font-weight: 700; color: rgba(255,255,255,0.35); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
...
.modal-tag { background: rgba(212,255,0,0.1); color: rgba(212,255,0,0.8); padding: 4px 12px; border-radius: 999px; font-size: 0.75rem; font-weight: 600; }
.modal-actions { display: flex; gap: 12px; justify-content: flex-end; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.08); }
.btn-close { padding: 11px 20px; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: white; border-radius: 10px; cursor: pointer; font-weight: 600; font-family: inherit; }
.btn-open { padding: 11px 22px; background: linear-gradient(135deg, #D4FF00, #a8cc00); color: #0b0f2a; border: none; border-radius: 10px; cursor: pointer; font-weight: 800; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; font-family: inherit; }
